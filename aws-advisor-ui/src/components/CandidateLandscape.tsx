...
function bubbleRadius(price: number | null | undefined, minPrice: number, maxPrice: number): number {
  if (price == null || price <= 0) return 3;
  if (minPrice === maxPrice) return 6;
  const normalized = (price - minPrice) / (maxPrice - minPrice);
  return 3 + Math.sqrt(Math.max(0, Math.min(1, normalized))) * 7;
}
...
export function CandidateLandscape({
  ...
  const prices = candidates
    .map((candidate) => candidate.hourly_price_usd)
  ...
  const recommended = candidates.find((candidate) => candidate.instance_type === recommendedInstance);
  const alternative = alternativeInstance
    ? candidates.find((candidate) => candidate.instance_type === alternativeInstance)
    : undefined;
  const recommendedPoint = recommended
    ? { x: scaleX(recommended.vcpu), y: scaleY(recommended.memory_gib) }
    : null;
  const alternativePoint = alternative
    ? { x: scaleX(alternative.vcpu), y: scaleY(alternative.memory_gib) }
    : null;
  const candidateDistance = recommendedPoint && alternativePoint
    ? Math.hypot(recommendedPoint.x - alternativePoint.x, recommendedPoint.y - alternativePoint.y)
    : 0;
    ...
    <div className="w-full overflow-x-auto">
      ...
      >
        <g transform={`translate(${MARGIN.left},${MARGIN.top})`}>
          ...
          />
          {xTicks.map((v, index) => (
            <g key={`x${index}-${v}`} transform={`translate(${scaleX(v)},${plotH})`}>
          ...
          />
          {yTicks.map((v, index) => (
            <g key={`y${index}-${v}`} transform={`translate(0,${scaleY(v)})`}>
          ...
          {recommendedPoint && alternativePoint && candidateDistance > 8 && (
            <line
              x1={recommendedPoint.x}
              y1={recommendedPoint.y}
              x2={alternativePoint.x}
              y2={alternativePoint.y}
              stroke={C.amberDim}
              strokeWidth={1}
              strokeDasharray="3 3"
              aria-label="recommended to alternative connector"
            />
          )}
          {candidates
            .filter(
            ...
            .map((c) => (
              <circle
                ...
                r={bubbleRadius(c.hourly_price_usd, minPrice, maxPrice)}
                fill={C.dotDefault}
                ...
                onMouseEnter={() => {
                  setTooltip({
                    ...
                    label: c.hourly_price_usd != null
                      ? `${c.instance_type} $${c.hourly_price_usd.toFixed(3)}/hr`
                      : c.instance_type,
                  });
                }}
              />
            ...
            candidates
              ...
              .map((c) => (
                <circle
                  ...
                  cx={scaleX(c.vcpu)}
                  cy={scaleY(c.memory_gib)}
                  r={bubbleRadius(c.hourly_price_usd, minPrice, maxPrice) + 2}
                  fill={C.amberDim}
                  style={{ cursor: "pointer" }}
                  onMouseEnter={() =>
                    setTooltip({
            ...
            .map((c) => (
              <g key={c.instance_type}>
                ...
                <circle
                  cx={scaleX(c.vcpu)}
                  cy={scaleY(c.memory_gib)}
                  r={bubbleRadius(c.hourly_price_usd, minPrice, maxPrice) + 3}
                  fill="none"
                  stroke={C.amberFaint}
                  strokeWidth={1}
                />
                <circle
                  cx={scaleX(c.vcpu)}
                  cy={scaleY(c.memory_gib)}
                  r={bubbleRadius(c.hourly_price_usd, minPrice, maxPrice) + 1}
                  fill={C.amber}
                  style={{ cursor: "pointer" }}
                  onMouseEnter={() =>
                    setTooltip({
        ...
        <g transform={`translate(${MARGIN.left + plotW - 110}, ${MARGIN.top + 4})`}>
          ...
          {prices.length > 0 && (
            <text x={14} y={alternativeInstance ? 60 : 43} fill={C.axisLabel} fontSize={9} fontFamily={C.font}>
              size = hourly price
            </text>
          )}
        </g>
