...
import { AnimatedNumber, useCountUp } from "./AnimatedNumber";
...
export function ScalingRangeBar({ minInstances, maxInstances, scalingRecommendation }: Props) {
  ...
  const animatedMin = useCountUp(minInstances);
  const animatedMax = useCountUp(maxInstances);

  if (singleInstance) {
    return (
      <div className="w-full">
        <span
          className="inline-block border border-amber/30 px-2 py-1 font-mono text-xs text-amber"
          aria-label={`Fixed: ${minInstances} instances`}
        >
          Fixed: <AnimatedNumber value={minInstances} /> {minInstances === 1 ? "instance" : "instances"}
        </span>
        <p
  ...
  const minX = (animatedMin / scale) * fullW;
  const maxX = (animatedMax / scale) * fullW;
