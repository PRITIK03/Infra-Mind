...
interface Props {
  ...
  technicalNeeds?: Pick<TechnicalNeeds, "min_instances" | "max_instances">;
}
...
export function TopologyDiagram({ sdr, technicalNeeds }: Props) {
          ...
          return (
            <g key={`line-${i}`}>
              ...
              <polygon
                ...
                fill={C.line}
              />
              {nodes[i].id === "lb" && nodes[i + 1].id === "compute" && technicalNeeds && (
                <text
                  x={(x1 + x2) / 2}
                  y={cy - 7}
                  textAnchor="middle"
                  ...
                  fontFamily={C.font}
                >
                  {technicalNeeds.min_instances}-{technicalNeeds.max_instances}
                </text>
              )}
            </g>
          );
